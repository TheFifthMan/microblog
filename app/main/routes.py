...
def index():
    ...
    posts = current_user.followed_posts().paginate(page,current_app.config['PAGINATE_PER_PAGE'],False)
...
def explore():
    page = request.args.get('page',1,type=int)
    posts = Post.query.order_by(Post.timestamp.desc()).paginate(page,current_app.config['PAGINATE_PER_PAGE'],False)
